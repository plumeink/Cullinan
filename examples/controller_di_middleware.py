"""Controller + DI + Middleware example

Demonstrates:
- Registering providers with ProviderRegistry
- Adding ProviderRegistry to InjectionRegistry
- Using @injectable and Inject for property injection
- Using RequestScope to provide per-request instances
- Emulating middleware by creating request context in handler.prepare()

This script starts a Tornado server, performs two requests to the same endpoint
and verifies that request-scoped instance is new per request while singleton service
