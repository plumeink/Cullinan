...
> **当前版本：0.93a5**。当前文档已对齐 application-first 启动模型（`Application` + `@module`）、统一的 `cullinan.core` 容器门面、传输无关的 Web Runtime（`WebRequest` / `WebResponse` / `WebAdapter`）以及当前以 pytest 为主的测试工作流。
